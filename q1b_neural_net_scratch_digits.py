...
class ScratchNeuralNetworkDigits:
    ...
    def train(self, training_images: np.ndarray, training_labels: np.ndarray) -> None:
        ...
        for i in range(self.epochs):
            output = self.forward(training_images)                          # Forward pass
            loss = self.loss(training_labels_flat, output)                  # Loss function
            grads = self.backward(training_images, training_labels_flat)    # Backward prop
            self.update_weights(grads)                   # Update weights

            # Print loss every 10 epochs (as benchmark)
